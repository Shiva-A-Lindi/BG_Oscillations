...
receiving_class_dict, FR_ext_all_nuclei = set_connec_ext_inp(Act[state], A_mvt,D_mvt,t_mvt,dt, N, N_real, K_real, receiving_pop_list, nuclei_dict,t_list, 
                                          ...
                                          use_saved_FR_ext= False, normalize_G_by_N=True, state = state)
# pickle_obj(FR_ext_all_nuclei, os.path.join(path, 'FR_ext_' + name + '_tau_m_' + str(neuronal_consts[name]['membrane_time_constant']['mean']).replace('.','-') + '.pkl'))


# FR_ext_all_nuclei = load_pickle(os.path.join(path, 'FR_ext_' + name + '_tau_m_' + str(neuronal_consts[name]['membrane_time_constant']['mean']).replace('.','-') + '.pkl'))
...
#                                           all_FR_list = all_FR_list , n_FR =n_FR, if_plot = if_plot, end_of_nonlinearity = 35, use_saved_FR_ext= True,
#                                           set_FR_range_from_theory=False, method = 'collective', normalize_G_by_N=True, FR_ext_all_nuclei_saved=FR_ext_all_nuclei)

print("rest ext inp mean = ", np.average(nuc[0].rest_ext_input))
print("FR_ext mean = ", np.average(nuclei_dict[name][0].FR_ext))
...
receiving_class_dict  = set_connec_ext_inp(Act[state], A_mvt,D_mvt,t_mvt,dt, N, N_real, K_real, receiving_pop_list, nuclei_dict,t_list, 
                                          all_FR_list = all_FR_list , n_FR =n_FR, if_plot = False, end_of_nonlinearity = end_of_nonlinearity[name][state], 
...
G[(name2, name1)] , G[(name3, name2)] , G[(name1, name3)] = 0.1 * 2 * g, g, 0.5 * g
...
#                 orientation='portrait', transparent=True ,bbox_inches = "tight", pad_inches=0.1)
smooth_pop_activity_all_nuclei(nuclei_dict, dt, window_ms = 5)
status = 'Pallidostratal_no_oscil'
# status = 'Pallidostratal_oscil'

fig = plot(nuclei_dict,color_dict, dt, t_list, A, A_mvt, t_mvt, D_mvt, ax = None, title_fontsize=15, plot_start = plot_start, title = '',
           include_FR = False, include_std=False, plt_mvt=False, legend_loc='upper right', ylim = [-10,70])
# fig = plot(nuclei_dict,color_dict, dt, t_list, A, A_mvt, t_mvt, D_mvt, ax = plt.gca(), title_fontsize=15, plot_start = plot_start, title = '',
#            include_FR = False, include_std=False, plt_mvt=False, legend_loc='upper right', ylim = None, plot_filtered=True, low_f = 8, high_f = 20)
...
fig.savefig(os.path.join(path, 'SNN_firing_'+status+'.pdf'), dpi = 300, facecolor='w', edgecolor='w',
                orientation='portrait', transparent=True ,bbox_inches = "tight", pad_inches=0.1)
fig_ = raster_plot_all_nuclei(nuclei_dict, color_dict, dt, outer = None, fig = None,  title = '', plot_start = plot_start_raster, plot_end = t_sim,
                            labelsize = 20, title_fontsize = 25, lw  = 2, linelengths = 2, n_neuron = 40, include_title = True,
                            set_xlim=True, ax_label = True,ylabel_x = 0.05)
fig_.set_size_inches((11, 7), forward=False)
fig_.savefig(os.path.join(path, 'SNN_raster_'+status+'.png'), dpi = 300, facecolor='w', edgecolor='w',
                orientation='portrait', transparent=True ,bbox_inches = "tight", pad_inches=0.1)
fig_.savefig(os.path.join(path, 'SNN_raster_'+status+'.pdf'), dpi = 300, facecolor='w', edgecolor='w',
...
##### D2 RMP var = 1 t = 5s
# 2*g,0.5* g, 0.5*g, g, g, -g * 3, g * 3 ,g * 0.1 --> 3p Hz
...
status = 'Pallidostratal_plus_STN_GPe'
fig = plot(nuclei_dict,color_dict, dt, t_list, A, A_mvt, t_mvt, D_mvt, ax = None, title_fontsize=15, plot_start = plot_start, title = '',
           include_FR = False, include_std=False, plt_mvt=False, legend_loc='upper right')
fig = plot(nuclei_dict,color_dict, dt, t_list, A, A_mvt, t_mvt, D_mvt, ax = plt.gca(), title_fontsize=15, plot_start = plot_start, title = '',
           include_FR = False, include_std=False, plt_mvt=False, legend_loc='upper right',  plot_filtered=True, low_f = low_f, high_f = high_f)
...
ax.set_xlim(5,55)
ax.axvspan(5,55, alpha = 0.2, color = 'darkseagreen')
fig.savefig(os.path.join(path, 'SNN_spectrum_DD_'+status+'.png'), dpi = 300, facecolor='w', edgecolor='w',
                orientation='portrait', transparent=True ,bbox_inches = "tight", pad_inches=0.1)
fig.savefig(os.path.join(path, 'SNN_spectrum_DD_'+status+'.pdf'), dpi = 300, facecolor='w', edgecolor='w',
                orientation='portrait', transparent=True ,bbox_inches = "tight", pad_inches=0.1)
fig, ax = plt.subplots(1,1)
peak_threshold = 0.1; smooth_window_ms = 3 ;smooth_window_ms = 5 ; cut_plateau_epsilon = 0.1; lim_oscil_perc = 10; low_pass_filter = False
find_freq_SNN_not_saving(dt, nuclei_dict, duration_base, lim_oscil_perc, peak_threshold , smooth_kern_window , smooth_window_ms, cut_plateau_epsilon , False , 'fft' , False , 
...
# n_run = 10; plot_firing = False; plot_spectrum= False; plot_raster = False; plot_phase = False; low_pass_filter= False ;save_pkl = True ; save_figures = False; save_pxx = False
...
## Run on previously saved data
FR_ext_all_nuclei  = load_pickle( os.path.join(path, 'FR_ext_STN-Proto-Arky-D2-FSI.pkl'))
receiving_class_dict  = set_connec_ext_inp(Act[state], A_mvt,D_mvt,t_mvt,dt, N, N_real, K_real, receiving_pop_list, nuclei_dict,t_list, 
                                          all_FR_list = all_FR_list , n_FR =n_FR, if_plot = False, end_of_nonlinearity = end_of_nonlinearity, 
                                          set_FR_range_from_theory=False, method = 'collective', return_saved_FR_ext= False, 
                                          use_saved_FR_ext= True, FR_ext_all_nuclei_saved=FR_ext_all_nuclei, normalize_G_by_N=True)
...
filename = os.path.join(path, 'Beta_power','STN_D2_Proto_FSI_Arky_N_1000_T_5000_G_all_changing_1_pts_10_runs_14-3Hz-2.pkl' )
y_max_series = {'D2':0.8, 'STN': 4, 'Arky': 4, 'Proto': 8, 'FSI': 6}
...
fig = phase_summary(filename, name_list, color_dict, n_g_list, ref_nuc_name= 'D2', 
                    shift_phase = shift_phase, set_ylim = True, y_max_series=y_max_series)

figname = filename.split('.')[0] + '_Phase'
save_pdf_png(fig, figname, size = (5, len(name_list) * 4))
...
fig = PSD_summary(filename, name_list, color_dict, n_g_list, xlim = (8,50), inset_props = inset_props)#, inset_yaxis_loc = 'left')

figname = filename.split('.')[0] + '_PSD'
save_pdf_png(fig, figname, size = (6,4))
...
(synaptic_time_constant[(name2 , name3)], 
 ...
 synaptic_time_constant[(name1 , name2)])  =  [15],[6],[10]
...
filename = ( 'data_synaptic_weight_D2-P-' + name3[0] + '_tau_ratio_' + 
            str(syn_decay_dict['tau_1']['tau_ratio'][(name2 , name3)]) + '_' + 
            str(syn_decay_dict['tau_1']['tau_ratio'][(name3 , name1)]) + '_' + 
            str(syn_decay_dict['tau_1']['tau_ratio'][(name1 , name2)]) + '.pkl' )
receiving_class_dict = set_connec_ext_inp(A, A_mvt,D_mvt, t_mvt,dt, N, N_real, K_real, receiving_pop_list, nuclei_dict,t_list)
...
filename = ( 'G_sweep_D2-P-' + name3[0] + '_tau_' + 
            str(synaptic_time_constant[(name2 , name3)][0]) + '_' + 
            str(synaptic_time_constant[(name3 , name1)][0]) + '_' + 
...
xlabel = r'$G_{Arky-P}=5\times G_{D2-Arky}=2\times G_{P-D2}$' 
xlabel = r'$G_{D2-FSI}=G_{FSI-P}=\frac{G_{P-D2}}{2}$' 

fig = scatter_2d_plot(np.squeeze(data['g'][:,:,1]),
                      np.squeeze(data[(name,param)]),
                      np.squeeze(data[(name,color)]), 
                      name + ' in Pallidostriatal',  
                      [xlabel, param, color] )
...
#%% RATE MODEL : Any Pallidostriatal with GPe-GPe sweep
...
filename = ( 'data_synaptic_weight_D2-P-' + name3[0] + 'A_tau_' + 
            str(synaptic_time_constant[(name2 , name3)][0]) + '_' + 
            str(synaptic_time_constant[(name3 , name1)][0]) + '_' + 
            str(synaptic_time_constant[(name1 , name2)][0])+'.pkl' )
...
fig = scatter_3d_wireframe_plot(
                        data['g'][:,:,0],
                          data['g'][:,:,1],
                          data[(name,param)],
                          data[(name,color)], 
                          name, 
                          [r'$G_{Proto-Proto}$', xlabel, 
                           param,  color]
                          ) 
fig = scatter_2d_plot(np.squeeze(data['g'][:,:,1]),
                      np.squeeze(data[(name,param)]),
...
#%% RATE MODEL : frequency vs. G ( FSI loop)
g_cte_ind = [0,0,0]; g_ch_ind = [1,1,1]
nucleus_name_list = ['FSI', 'Proto','D2']
####### with GP-GP
...
x_label = r'$G_{D2-FSI}=G_{FSI-P}=\frac{G_{P-D2}}{2}$'
synaptic_weight_transition_multiple_circuits(filename_list, nucleus_name_list, legend_list, 
                                             color_list,g_cte_ind,g_ch_ind,param_list,color_param_list,'hot',
                                             x_axis = 'g_2',title = title,x_label = x_label)
...
#%% RATE MODEL : synaptic weight phase exploration only GP
# T[('Proto', 'Proto')]= 2
T[('Proto', 'Proto')]= 5
...
plt.axvline(g_transient[1], c = 'k')
#%% RATE MODEL : synaptic weight phase exploration only STN
plt.close('all')
N_sim = 100
N = dict.fromkeys(N, N_sim)
t_sim = 400 # simulation time in ms
...
#%% RATE MODEL : frequency vs. G ( STN-GP and GP-GP )
plt.close('all')
...
fig.savefig(os.path.join(path_rate, filename.replace('.png','.pdf')),dpi = 300, facecolor='w', edgecolor='w',
                            orientation='portrait', transparent=True ,bbox_inches = "tight", pad_inches=0.1)
#%% RATE MODEL : frequency vs. G ( FSI and Arky loops)

plt.close('all')
g_cte_ind = [0,0]; g_ch_ind = [1,1]
# color_list = [color_dict[name] for name in nucleus_name_list]
filename_list = ['data_synaptic_weight_Pallidostriatal_tau_15_6_10.pkl',
                 'data_synaptic_weight_D2-P-A_tau_15_6_10.pkl']
...
#%% RATE MODEL : STN-GPe + GP-GP time scale space GABA and glut
...
#%% RATE MODEL : STN-GPe alone time scale space GABA and glut
N_sim = 100
N = dict.fromkeys(N, N_sim)
t_sim = 800 # simulation time in ms
dt = 0.1 # euler time step in ms
...
#%% RATE MODEL : different sets of parameters vs. G
#################### All circuits
...
fig.savefig(figname+'.png',dpi = 300)
fig.savefig(figname+'.pdf',dpi = 300)
#%% RATE MODEL : dependency of three loops on tau_inhibition
