...
N_real = {'STN': 13560, 'Proto': 46000*0.70,
          'Arky': 46000*0.25, 'GPi': 3200, 'Str': N_Str,
...
A_anesthetized = {'STN': 7,  # De la Crompe (2020) averaged see cell: De La Crompe FR distributions / Mallet et al. 2008 (Sharott) from Fig 1I ~ 14 +/- 3 Hz
                  ...
                  'FSI': 3.67, # Mallet et al. 2005 Table 2 (SW-) sd = +/- 1.14 / # Sharott et al. 2012 Fig 5A (SW-)
...
neuronal_consts = {
    ...
    'Proto': {
        ...
        'nonlin_thresh': -20, 'nonlin_sharpness': 1, 'u_initial': {'min': -66.3, 'max': -44.},
        
        # 'u_rest': {'mean': -53, 'var': 4 , 'truncmin': -1000, 'truncmax': -45}, 
        # Bugaysen et al. 2010 [type B] Vm (rest?) -53 +/- 4 n= 24 temp = 34 (P12-P21)
        'u_rest': {'mean': -66.3, 'var': 9.19 , 'truncmin': -100, 'truncmax': -60}, 
        ...
        # Abdi et al 2015 Table. 1 : average of "mean membrane potential" PV+ and PV- proto is -67.08 +/- 6.45 n = 19 juvenile rat (P20-P35) in vitro electric stim temp = 35-37


        # 'membrane_time_constant':{'mean': 25.4,'var': 12, 'truncmin': 3, 'truncmax': 80}, 
        # Stanford & cooper 2000 [type A] 25.4 +/- 11.49 sem = 1.0, n = 132 Table 2. temp = 32-34 in vitro whole cell patch
        # 'membrane_time_constant': {'mean': 43, 'var': 10, 'truncmin': 0.5, 'truncmax': 100}, 
        # Jerome's measurements temp = 32
        'membrane_time_constant':{'mean':12.94,'var': 10.23, 'truncmin': 3, 'truncmax': 100},
        ...
        # Abdi et al 2015 Table. 1 : average of PV+ and PV- proto n = 19 juvenile rat (P20-P35) in vitro electric stim temp = 35-37 
    ...
    'Arky': {
        ...
        # 'u_rest': {'mean': -70, 'var': 1., 'truncmin': -1000, 'truncmax': -56},  
        # Abdi et al 2015 from trace juvenile rat (P20-P35) in vitro electric stim temp = 35-37
        # 'u_rest': {'mean': -58.1, 'var': 8.93, 'truncmin': -1000, 'truncmax': -50},
        ...
        # Jerome's measurements temp = 32
        'membrane_time_constant':{'mean':19.9,'var': 13}, 
        ...
        # 'spike_thresh': {'mean': -55.0, 'var': 7.64}},  
        # Abdi et al 2015 Table. 1 sem = 1.8 n = 18  (P20-P35) rat in vitro electric stim temp = 35-37
    ...
    'D2': {
        ...
        'nonlin_thresh': -20, 'nonlin_sharpness': 1, 'u_initial': {'min': -78.5, 'max': -51},
        ...
        # 'u_rest': {'mean': -76.8 , 'var': 4.43, 'truncmin': -1000, 'truncmax': -42}, 
        ...
        'membrane_time_constant': {'mean': 4.9, 'var': 1.58, 'truncmin': 0.5, 'truncmax': 100},  
    ...
    'FSI': {
        ...
        'u_rest': {'mean': -78.2, 'var': 6.2, 'truncmin': -1000, 'truncmax': -54},  
        ...
        'membrane_time_constant': {'mean': 3.1, 'var': 1.1, 'truncmin': 0.5, 'truncmax': 100},  
        ...
        'spike_thresh': {'mean': -52.4, 'var': 4.6 }},  
    ...
    'STN': {
        ...
        'nonlin_thresh': -20, 'nonlin_sharpness': 1, 'u_initial': {'min': -59, 'max': -41.4},
        'u_rest': {'mean': -59, 'var': 1.2, 'truncmin': -1000, 'truncmax': -43},  
        ...
        'membrane_time_constant': {'mean': 5.13, 'var': 2.38 , 'truncmin': 0.5, 'truncmax': 100},  
        ...
        # 'spike_thresh': {'mean': -50.8, 'var': 1.5}}}  
...
K_real = {('STN', 'Proto'): int(243*N_real['Proto']/N_real['STN']),  # 243 bouton per GP. number of synapses per bouton = 12/10.6  Baufreton et al. 2009 & Sadek et al. (2006).
          ...
          ('Proto', 'D2'): int(N_real['D2']*226/N_real['Proto']),
          # each Proto 123 from dSPN Kawaguchi et al. (1990)
          ('Proto', 'D1'): int(N_real['D1']*123/N_real['Proto']),
          ('GPi', 'D1'): 1,  # find !!!!
          # boutons Kita & Jaeger (2016) based on Koshimizu et al. (2013)
          ('GPi', 'STN'): 457,
          ('GPi', 'Proto'): 1,  # find !!!
...
K_all = {'rest': K_real, 'DD_anesth': K_real_DD, 'awake_rest': K_real, 'mvt': K_real}
...
FR_ext_sd_dict = {   
    'Proto': {'rest' : 0.000001, 'awake_rest' : 0, 'DD_anesth' : 0, 'mvt' : 0, 'trans_Nico_mice' : 0,'trans_Kita_rat' : 0},
    'STN': {'rest' : 0, 'awake_rest' : 0, 'DD_anesth' : 0, 'mvt' : 0, 'trans_Nico_mice' : 0,'trans_Kita_rat' : 0},
    'FSI': {'rest' : 0, 'awake_rest' : 0, 'DD_anesth' : 0, 'mvt' : 0, 'trans_Nico_mice' : 0,'trans_Kita_rat' : 0},
    'D2': {'rest' : 0, 'awake_rest' : 0, 'DD_anesth' : 0, 'mvt' : 0, 'trans_Nico_mice' : 0,'trans_Kita_rat' : 0},
    'Arky': {'rest' : 0, 'awake_rest' : 0, 'DD_anesth' : 0, 'mvt' : 0, 'trans_Nico_mice' : 0,'trans_Kita_rat' : 0}
                }
...
nuc = [Nuc_keep_V_m(i, gain, threshold, neuronal_consts, tau, ext_inp_delay, noise_variance[state], noise_amplitude, 
                    ...
                    FR_ext_specs=FR_ext_specs[name][state], plot_spike_thresh_hist=True) for i in pop_list]
...
for name in list(nuclei_dict.keys()):
    print('mean I0 ', name, np.average(nuclei_dict[name][0].rest_ext_input))
...
# fig_FR_dist = plot_FR_distribution(nuclei_dict, dt, color_dict, # bins = np.logspace(-2, 2, 50)
#                                     bins = np.arange(0, bins[name][state]['max'], bins[name][state]['step']),
...
# tau[('STN', 'Proto')] =  {'rise': [1.1], 'decay': [10]}
# tau[('Arky', 'Proto')] =  {'rise': [1.1], 'decay': [8]}
# tau[('Proto', 'D2')] =  {'rise': [1.1], 'decay': [30]}
...
proto_shift = 0
sheet_name_extra = ''
time_shift_dict = {'MSNs': 2.6 ,'STN': 2.6 - 10.1 , 'Arky': 2.6 - 10.1, 'Proto': 2.6 - 10.1}
...
ax.set_xlim(-15, 150)
save_pdf_png(fig, os.path.join(path, 'Real_D2_trans_stim_onto_Proto' + str(n_run) + '_run_compared_with_experiment_' + filename.split('.')[0].split('_')[-1] ), size = (5,3))
...
t_list = np.arange(int(t_sim/dt))
plot_start = 300
t_transition = plot_start + int(t_sim / 5)
duration_base = np.array( [int(300/dt), int(t_transition/dt)] )
duration_DD = np.array( [int(t_transition / dt) + int(500/dt) , int(t_sim / dt)] ) 

n_windows = 4
plot_start = t_transition - 700
plot_end = t_transition + 700
...
if 'DD' in state_2:
    fig = plot(nuclei_dict, color_dict, dt, t_list, Act[state_1], Act[state_2], t_transition, D_mvt, ax=None, title_fontsize=15, 
               plot_start= t_transition-400, title='', legend_loc='upper left', plot_end=t_transition-10, ylim=ylim,
               include_FR=False, continuous_firing_base_lines=False, plt_mvt=True, alpha_mvt=0.8, ncol_legend = 1)
    save_pdf_png(fig, os.path.join(path, 'SNN_firing_' + status + '_' + state_1),
                 size=(8*4/7, 5))
    
    fig = plot(nuclei_dict, color_dict, dt, t_list, Act[state_1], Act[state_2], t_transition, D_mvt, ax=None, title_fontsize=15, 
               plot_start = t_sim - 700, title='', 
               legend_loc='upper left', plot_end= t_sim, vspan=True, ylim=ylim,
               include_FR=False, continuous_firing_base_lines=False, plt_mvt=True, alpha_mvt=0.8, 
    ...
    save_pdf_png(fig, os.path.join(path, 'SNN_firing_' + status + '_plot_' + state_2),
                  size=(8, 5))
    fig_state_1, fig_state_2 = raster_plot_all_nuclei_transition(nuclei_dict, color_dict, dt, outer=None, fig=None,  title='', plot_start=plot_start,
                                             labelsize=20, title_fontsize=15, lw=1., linelengths=2, n_neuron=40, include_title=False, set_xlim=True,
                                             axvspan_color=axvspan_color[state_2], n=N_sim,  ylabel_x=0.01,
                                             t_transition=t_transition, t_sim=t_sim, tick_label_fontsize=15, include_nuc_name=False)
...
#### transition
receiving_class_dict, nuclei_dict = set_connec_ext_inp(path, Act[state_2], A_mvt, D_mvt, t_mvt, dt, N, N_real, K_all[state_2], receiving_pop_list, nuclei_dict, t_list,
                                          all_FR_list=all_FR_list, n_FR=n_FR, if_plot=False, end_of_nonlinearity=end_of_nonlinearity,
                                          set_FR_range_from_theory=False, method='collective',  save_FR_ext= False,
...
figs, title, data = synaptic_weight_exploration_SNN(path, nuclei_dict, filepath, duration, G_dict, color_dict, dt, t_list, Act[state_2], A_mvt, t_mvt, D_mvt, receiving_class_dict,
                                                    ...
                                                    reset_init_dist=True, all_FR_list=all_FR_list, n_FR=n_FR, if_plot=False, end_of_nonlinearity=end_of_nonlinearity,
                                                    state=state_2, K_real=K_all[state_2], N_real=N_real, N=N, divide_beta_band_in_power=True,
                                                    receiving_pop_list=receiving_pop_list, poisson_prop=poisson_prop, return_saved_FR_ext=False,
                                                    use_saved_FR_ext=True, check_peak_significance=check_peak_significance,
...
nuc_order = ['Proto']
ref_nuc_name = 'Proto'
figs, title, data = synaptic_weight_exploration_SNN(path, nuclei_dict, filepath, duration_base, G_dict, color_dict, dt, t_list, Act[state], A_mvt, t_mvt, D_mvt, receiving_class_dict,
                                                    noise_amplitude, noise_variance, lim_oscil_perc=10, plot_firing=plot_firing, low_pass_filter=low_pass_filter, legend_loc=legend_loc,
                                                    lower_freq_cut=8, upper_freq_cut=40, set_seed=False, firing_ylim=None, n_run=n_run,  plot_start_raster=plot_raster_start,
                                                    plot_spectrum=plot_spectrum, plot_raster=plot_raster, plot_start=plot_start, plot_end=t_sim, n_neuron=n_neuron, round_dec=round_dec, include_std=include_std,
                                                    find_beta_band_power=True, fft_method=fft_method, n_windows=3, include_beta_band_in_legend=True, save_pkl=save_pkl,
...
ref_nuc_name = 'Proto'
nuc_order = ['Proto', 'STN']
figs, title, data = synaptic_T_exploration_SNN(path, nuclei_dict, filepath, duration_base, G, T_dict, color_dict, dt, t_list, Act[state], A_mvt, t_mvt, D_mvt, receiving_class_dict,
                                                    noise_amplitude, noise_variance, lim_oscil_perc=10, plot_firing=plot_firing, low_pass_filter=low_pass_filter, legend_loc=legend_loc,
                                                    lower_freq_cut=8, upper_freq_cut=40, set_seed=False, firing_ylim=None, n_run=n_run,  plot_start_raster=plot_raster_start,
                                                    plot_spectrum=plot_spectrum, plot_raster=plot_raster, plot_start=plot_start, plot_end=t_sim, n_neuron=n_neuron, round_dec=round_dec, include_std=include_std,
                                                    find_beta_band_power=True, fft_method=fft_method, n_windows=3, include_beta_band_in_legend=True, save_pkl=save_pkl,
...
# %% Synapric weight exploraion 2d FSI-D2-GPe + GPe-GPe resetting inti dists and setting ext input collectively
...
ref_nuc_name = 'D2'
figs, title, data = synaptic_weight_exploration_SNN_2d(loop_key_lists, path, nuclei_dict, filepath, duration_base, G_dict, color_dict, dt, t_list, Act[state], A_mvt, t_mvt, D_mvt, receiving_class_dict,
                                                    noise_amplitude, noise_variance, lim_oscil_perc=10, plot_firing=plot_firing, low_pass_filter=low_pass_filter, legend_loc=legend_loc,
                                                    lower_freq_cut=8, upper_freq_cut=40, set_seed=False, firing_ylim=None, n_run=n_run,  plot_start_raster=plot_raster_start,
                                                    plot_spectrum=plot_spectrum, plot_raster=plot_raster, plot_start=plot_start, plot_end=t_sim, n_neuron=n_neuron, round_dec=round_dec, include_std=include_std,
                                                    find_beta_band_power=True, fft_method=fft_method, n_windows=3, include_beta_band_in_legend=False, save_pkl=save_pkl,
...
# %% Synapric weight exploraion 2d FSI-D2-GPe + STN-GPe resetting inti dists and setting ext input collectively
...
FR_ext_all_nuclei = load_pickle(
    os.path.join(path, 'FR_ext_STN-Proto-Arky.pkl'))
receiving_class_dict = set_connec_ext_inp(Act[state], A_mvt, D_mvt, t_mvt, dt, N, N_real, K_real, receiving_pop_list, nuclei_dict, t_list,
                                          all_FR_list=all_FR_list, n_FR=n_FR, if_plot=False, end_of_nonlinearity=end_of_nonlinearity,
                                          set_FR_range_from_theory=False, method='collective', return_saved_FR_ext=False,
                                          use_saved_FR_ext=True, FR_ext_all_nuclei_saved=FR_ext_all_nuclei, normalize_G_by_N=True)
...
filename = 'STN_Proto_Arky_N_1000_T_2000_' + \
    str(n) + '_pts_' + str(n_run) + '_runs' + '.pkl'
...
nuclei_dict = {name:  [Nucleus(i, gain, threshold, neuronal_consts, tau, ext_inp_delay, noise_variance[state], noise_amplitude, N, A, A_mvt, name, G, T, t_sim, dt,
               synaptic_time_constant, receiving_pop_list, smooth_kern_window, oscil_peak_threshold, neuronal_model='spiking', set_input_from_response_curve=set_input_from_response_curve,
               poisson_prop=poisson_prop, init_method=init_method, der_ext_I_from_curve=der_ext_I_from_curve, mem_pot_init_method=mem_pot_init_method,  keep_mem_pot_all_t=keep_mem_pot_all_t,
               ext_input_integ_method=ext_input_integ_method, syn_input_integ_method=syn_input_integ_method, path=path,
               save_init=save_init, syn_component_weight=syn_component_weight, noise_method=noise_method) for i in pop_list] for name in name_list}
...
filename = 'STN_D2_Proto_FSI_Arky_N_1000_T_5000_G_all_changing_' + \
    str(n) + '_pts_' + str(n_run) + '_runs' + '.pkl'
...
figs, title, data = synaptic_weight_exploration_SNN(nuclei_dict, filepath, duration_base, G_dict, color_dict, dt, t_list, A, A_mvt, t_mvt, D_mvt, receiving_class_dict,
                                                    ...
                                                    use_saved_FR_ext=True, FR_ext_all_nuclei_saved=FR_ext_all_nuclei, check_peak_significance=False,
                                                    find_phase=True, phase_thresh_h=0, filter_order=6, low_f= low_f, high_f= high_f,
                                                    n_phase_bins=180, start_phase=int(t_sim/4), ref_nuc_name='D2', save_pxx=save_pxx,
                                                    plot_phase=plot_phase, total_phase=720, phase_projection=None, troughs=True,
                                                    nuc_order=nuc_order, save_pop_act=True)
...
y_max_series = {'D2': 0.2, 'STN': 1.5, 'Arky': 1, 'Proto': 1.3, 'FSI': .4}
n_g_list = np.array([0])
ref_nuc_name = 'D2'
...
fig = PSD_summary(filename, name_list, color_dict, n_g_list, xlim=(0, 80),# inset_props=inset_props,
                  ...
                  legend_loc='upper right', x_y_label_size = 10, tick_label_fontsize = 12,
...
state = 'rest'
t = 5000
filename_list = ['Proto_Proto_N_1000_T_'+ str(t) + '_1_pts_8_runs_dt_0-1_' + state + 
                      '_Ornstein-Uhlenbeck_A_' + get_str_of_A_with_state(['Proto'], Act, state),
                  'STN_Proto_N_1000_T_'+ str(t) + '_1_pts_8_runs_dt_0-1_' + state + 
                      '_Ornstein-Uhlenbeck_A_' + get_str_of_A_with_state(['Proto', 'STN'], Act, state),
                  'D2_Proto_FSI_N_1000_T_'+ str(t) + '_1_pts_8_runs_dt_0-1_' + state + 
                      '_Ornstein-Uhlenbeck_A_' + get_str_of_A_with_state(['FSI', 'D2', 'Proto'], Act, state),
                'D2_Proto_Arky_N_1000_T_'+ str(t) + '_1_pts_8_runs_dt_0-1_' + state + 
                      '_Ornstein-Uhlenbeck_A_' + get_str_of_A_with_state(['Arky', 'D2', 'Proto'], Act, state)]

filename_list = [os.path.join(path, 'Beta_power', file + '.pkl')
...
color_dict_ = {'Proto-Proto': color_dict['Proto'], 'STN-Proto' :color_dict['STN'],
...
for count, loop in enumerate(loop_list):
    ...
    color = color_dict_[loop]
    
    bp = ax.boxplot(freq, labels= list(xlabels.values()), patch_artist=True,
                    whis=(0, 100), zorder=0)
    for patch, color in zip(bp['boxes'], [color] * len(state_list)):
        patch.set_facecolor(color)
    ...
    for x, val, c in zip(xs, vals, [color] * len(state_list)):
        ax.scatter(x, val, c=c, alpha=0.4, s=10, ec='grey', zorder=1)
...
temp_oscil_check(nuclei_dict['STN'][0].pop_act,
                 oscil_peak_threshold['STN'], 3, dt, *duration_mvt)
...
fig_trans, fig_stable = synaptic_weight_space_exploration(G.copy(), Act[state_1], Act[state_2], D_mvt, t_mvt, t_list, dt,  filename,
...
nucleus = nuclei_dict[name][0]
fig.tight_layout()
find_freq_of_pop_act_spec_window(nucleus, *duration_base, dt, peak_threshold=nucleus.oscil_peak_threshold,
...
temp_oscil_check(nuclei_dict[name][0].pop_act,
                 oscil_peak_threshold[name], 3, dt, *duration_mvt)
...
receiving_pop_list = {('Arky', '1'): [('Proto', '1')],
                      ('Proto', '1'): [('D2', '1'), ('Proto', '1')],
                      ('D2', '1'): [('Arky', '1')]}
synaptic_time_constant[('D2', 'Proto')], synaptic_time_constant[('Arky', 'Proto')], synaptic_time_constant[(
    'Proto', 'D2')], synaptic_time_constant[('Proto', 'Proto')] = [30], [6], [10], [10]

pop_list = [1]
G_ratio_dict = {('D2', 'Arky'): .2, ('Arky', 'Proto'): 1,
                ('Proto', 'D2'): 0.5, ('Proto', 'Proto'): 1}
...
D2 = [Nucleus(i, gain, threshold, neuronal_consts, tau, ext_inp_delay, noise_variance, noise_amplitude, N, A, A_mvt, 'D2',
              G, T, t_sim, dt, synaptic_time_constant, receiving_pop_list, smooth_kern_window, oscil_peak_threshold)for i in pop_list]
Arky = [Nucleus(i, gain, threshold, neuronal_consts, tau, ext_inp_delay, noise_variance, noise_amplitude, N, A, A_mvt, 'Arky',
                G, T, t_sim, dt, synaptic_time_constant, receiving_pop_list, smooth_kern_window, oscil_peak_threshold)for i in pop_list]
lim_n_cycle = [6, 10]
nuclei_dict = {'Proto': Proto, 'D2': D2, 'Arky': Arky}
receiving_class_dict = set_connec_ext_inp(
    A, A_mvt, D_mvt, t_mvt, dt, N, N_real, K_real, receiving_pop_list, nuclei_dict, t_list)
filename = 'data_synaptic_weight_D2-P-A_with_P_P_tau_'+str(g_1_list[0])+'_'+str(synaptic_time_constant[('D2', 'Arky')][0])+'_'+str(
    synaptic_time_constant[('Arky', 'Proto')][0])+'_'+str(synaptic_time_constant[('Proto', 'D2')][0])+'_'+str(synaptic_time_constant[('Proto', 'Proto')][0])+'.pkl'
synaptic_weight_space_exploration(G.copy(), A, A_mvt, D_mvt, t_mvt, t_list, dt, filename, lim_n_cycle, G_dict, nuclei_dict,
                                  duration_mvt, duration_base, receiving_class_dict, color_dict, G_ratio_dict=G_ratio_dict, if_plot=if_plot)
...
scatter_2d_plot(np.squeeze(data['g'][:, :, 1]), np.squeeze(data[(name, param)]), np.squeeze(
    data[(name, color)]), name + ' in Pallidostriatal',  [r'$G_{D2-FSI}=G_{FSI-P}=\frac{G_{P-D2}}{2}$', param, color])
...
D2 = [Nucleus(i, gain, threshold, neuronal_consts, tau, ext_inp_delay, noise_variance, noise_amplitude, N, A, A_mvt, 'D2',
              G, T, t_sim, dt, synaptic_time_constant, receiving_pop_list, smooth_kern_window, oscil_peak_threshold)for i in pop_list]
FSI = [Nucleus(i, gain, threshold, neuronal_consts, tau, ext_inp_delay, noise_variance, noise_amplitude, N, A, A_mvt, 'FSI',
               G, T, t_sim, dt, synaptic_time_constant, receiving_pop_list, smooth_kern_window, oscil_peak_threshold)for i in pop_list]
lim_n_cycle = [6, 10]
...
sweep_time_scales(g_list, G_ratio_dict, synaptic_time_constant.copy(), nuclei_dict, syn_decay_dict, filename, G, A,
                  A_mvt, D_mvt, t_mvt, receiving_class_dict, t_list, dt, duration_base, duration_mvt, lim_n_cycle, find_stable_oscill)
...
receiving_pop_list = {('FSI', '1'): [('Proto', '1')],
                      ('Proto', '1'): [('D2', '1'), ('Proto', '1')],
                      ('D2', '1'): [('FSI', '1')]}
synaptic_time_constant[('D2', 'FSI')], synaptic_time_constant[('FSI', 'Proto')], synaptic_time_constant[(
    'Proto', 'D2')], synaptic_time_constant[('Proto', 'Proto')] = [30], [6], [10], [10]
...
run(receiving_class_dict, t_list, dt, nuclei_dict)
plot(nuclei_dict, color_dict, dt, t_list, A, A_mvt, t_mvt, D_mvt, ax=None,
     title=r"$G_{Proto-Proto}="+str(G[('Proto', 'Proto')])+"$" + r" $G_{D2-FSI}="+str(G[('D2', 'FSI')])+"$")
name = 'D2'
nucleus = nuclei_dict[name][0]
# find_freq_of_pop_act_spec_window(nucleus,*duration_base,dt, peak_threshold =nucleus.oscil_peak_threshold, smooth_kern_window = nucleus.smooth_kern_window, check_stability=True)
temp_oscil_check(nuclei_dict[name][0].pop_act,
...
ax2.set_xlim(4, 26)
ax2.set_ylim(5, 70)
ax2.legend(fontsize=18, frameon=False, framealpha=0.1,
