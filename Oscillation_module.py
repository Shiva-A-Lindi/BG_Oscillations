	...
	def __init__(self, population_number,gain, threshold,neuronal_consts,tau, ext_inp_delay, noise_variance, noise_amplitude, N, A,A_mvt, name, G, T, t_sim, dt, 
		...
		if neuronal_model == 'spiking':
			...
			if init_method == 'homogeneous':
				...
				self.initialize_heterogeneously( poisson_prop, dt, t_sim, spike_thresh_bound_ratio, *bound_to_mean_ratio,  plot_initial_V_m_dist = plot_initial_V_m_dist)
...
#    windowed = sig * signal.blackmanharris(len(sig))
	...
	if N == 0:
		...
		if plot_spectrum:
			plot_fft_spectrum( peak_freq, f, pxx, N, ax = ax, c = c, label = label, figsize = figsize, include_beta_band_in_legend = include_beta_band_in_legend)
		ind_max = 200
...
def freq_from_rfft(sig, dt, N):
	...
	# print(f)
	pxx = np.abs( rf[:N//2] ) ** 2
	peak_freq = f[np.argmax( abs( rf[ : N // 2] ) ) ]  # Just use this for less-accurate, naive version
...
def run_with_transient_external_input(receiving_class_dict,t_list, dt, nuclei_dict, rest_init_filepaths, 
	...
	selective_reset_ext_input(nuclei_dict, transient_init_filepaths, list_of_nuc_with_trans_inp, A_trans) #, A_mvt, D_mvt, t_mvt, t_list, dt)
	### interate through the duration of transient input
	iterate_SNN(nuclei_dict, dt,receiving_class_dict, t_start = t_transient, t_end = t_transient + duration)
	### reset back to rest 
	selective_reset_ext_input(nuclei_dict, rest_init_filepaths, list_of_nuc_with_trans_inp, A)#, A_mvt, D_mvt, t_mvt, t_list, dt)
	### look at the decay of the transient input
	iterate_SNN(nuclei_dict, dt,receiving_class_dict, t_start = t_transient + duration , t_end = t_list[-1])
